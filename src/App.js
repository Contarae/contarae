...
function ToolRenta({uv}){
  ...
  const chk=()=>{
    ...
    sRR({
      ...
      level:ob?"Obligación probable":near.length?"Caso cercano al tope":"Sin señales por topes",
...
function ToolRet({uv25,uv26}){
  ...
  const vals=yr===2025?{uv:uv25,smlmv:1423500}:{uv:uv26,smlmv:1750905};
  const {uv,smlmv}=vals;
