...
export default async (req) => {
  ...
  try {
    ...
    if (!reference || !key) {
      return new Response("Faltan parámetros para descargar el soporte.", {
        status: 400,
        headers: {
          "Cache-Control": "no-store"
        }
    ...
    if (!key.startsWith(`supports/${reference}/`)) {
      return new Response("Ruta de soporte inválida.", {
        status: 400,
        headers: {
          "Cache-Control": "no-store"
        }
    ...
    if (!blob) {
      return new Response("Soporte no encontrado.", {
        status: 404,
        headers: {
          "Cache-Control": "no-store"
        }
    ...
    const fileName = sanitizeFileName(String(blob.metadata?.originalName || key.split("/").pop() || "soporte"));
    ...
    return new Response(`No fue posible descargar el soporte. ${error.message}`, {
      status: 500,
      headers: {
        "Cache-Control": "no-store"
      }
